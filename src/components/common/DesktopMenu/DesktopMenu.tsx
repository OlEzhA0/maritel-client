        ...
        >
            <ul className="DesktopMenu__NavList">
                {categories.map((category) => (
                    <li className="DesktopMenu__NavItem" key={category._id}>
                        <Link
                            to={
                            ...
                            className={cn({
                                ...
                                "DesktopMenu__NavLink--hovered":
                                    category._id === hoveredItem,
                            })}
                            onMouseEnter={() => {
                                setIsHover(true);
                                setHoveredItem(category._id);
                            }}
                            onMouseLeave={() => {
                                setIsHover(false);
                                setHoveredItem("");
                            ...
                            category.subCategories.length > 0 && (
                                ...
                                >
                                    {category.subCategories.map((subCateg) => (
                                        <li
                                            key={subCateg.id}
                                            className="DesktopMenu__NavSubItem"
                                        >
                                            <Link
                                                to={`/${handleTranslit(
                                                    category.category
